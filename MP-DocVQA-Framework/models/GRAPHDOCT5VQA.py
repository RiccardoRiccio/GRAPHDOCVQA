...
class GRAPHDOCT5VQA(nn.Module):
    ...
    def process_batch_data(self, batch_data: dict) -> dict:
        """
        Convert the DataLoader batch into GraphDoc’s expected input:
          • batch_data['images']        : [B,3,512,512]
          • batch_data['questions']     : List[str]
          • batch_data['lines']         : List[List[str]]
          • batch_data['line_boxes_rs'] : [B, Nl_max, 4]
          • batch_data['line_mask']     : [B, Nl_max]
        Steps:
          1. Encode each question → [1, H_sent]
          2. Encode each group of OCR lines → [L_i, H_sent]
          3. Prepend the “global question” embedding + global bbox [0,0,512,512]
          4. Merge/pad → inputs_embeds: [B, L_tot_max, H_sent], attention_mask: [B, L_tot_max], bbox: [B, L_tot_max, 4]
          5. Return a dict suitable for `self.graphdoc(...)`
        """
        device = self.device
        B = len(batch_data["questions"])
        print(f"\n[process_batch_data] batch size B = {B}")


        images = batch_data["images"].to(device).float()        # [B, 3, 512, 512]
        line_boxes = batch_data["line_boxes_rs"].to(device)     # [B, Nl_max, 4]
        line_mask = batch_data["line_mask"].to(device)          # [B, Nl_max]

        # print(f"[process_batch_data] images.shape = {images.shape}")
        # print(f"[process_batch_data] line_boxes.shape = {line_boxes.shape}")
        # print(f"[process_batch_data] line_mask.shape = {line_mask.shape}")
        ...
        for i in range(B):
            # print(f"\n[process_batch_data] Processing sample index i={i}")
            # a) Encode OCR lines[i] via Sentence-BERT
            sample_lines = batch_data["lines"][i]  # List[str]
            # print(f"  [process_batch_data] sample_lines (len) = {len(sample_lines)}")
            if len(sample_lines) > 0:
                # extract_sentence_embeddings returns a NumPy array of shape [L_i, H_sent]
                np_embeds = extract_sentence_embeddings(
                    sample_lines,
                    self.sent_tokenizer,
                    self.sentence_bert
                )  # shape [L_i, hidden_size]

                # Convert back into a torch.Tensor on 'device'
                line_embeds = torch.from_numpy(np_embeds).to(device)  # [L_i, hidden_size]
                # print(f"  [process_batch_data] line_embeds.shape = {line_embeds.shape}")
            
            else:
                line_embeds = torch.zeros((0, self.gd_hidden), device=device)
                # print(f"  [process_batch_data] no OCR lines; line_embeds = zeros of shape {line_embeds.shape}")
            

            # b) Encode the question as a global node:
            question_embed = self.encode_question(batch_data["questions"][i])  # [1, H_sent]
            # print(f"  [process_batch_data] question_embed.shape = {question_embed.shape}")


            # c) Determine how many valid OCR lines (via line_mask)
            valid_lines = int(line_mask[i].sum().item())  # L_i
            # print(f"  [process_batch_data] valid_lines = {valid_lines}")

            # d) Build bounding boxes for this sample; global bbox = [0,0,512,512]
            global_box = torch.tensor([0, 0, 512, 512], dtype=torch.long, device=device)
            if valid_lines > 0:
                ent_boxes = line_boxes[i, :valid_lines]      # [L_i, 4]
                sample_boxes = torch.cat([global_box.unsqueeze(0), ent_boxes], dim=0)  # [1+L_i, 4]
                # print(f"  [process_batch_data] sample_boxes.shape = {sample_boxes.shape}")
            else:
                sample_boxes = global_box.unsqueeze(0)  # [1, 4]
                # print(f"  [process_batch_data] sample_boxes (no OCR) = shape {sample_boxes.shape}")

            # e) Combine question_embed (global) + OCR line_embeds
            if valid_lines > 0:
                sample_embeds = torch.cat([question_embed, line_embeds[:valid_lines]], dim=0)  # [1+L_i, H_sent]
                # print(f"  [process_batch_data] sample_embeds (with lines) shape = {sample_embeds.shape}")
            else:
        ...
        # print(f"\n[process_batch_data] Merging/padding {len(all_node_embeds)} node embedding lists…")

        # 2) Merge/pad them into final batch tensors:
        inputs_embeds = merge2d(all_node_embeds, pad_id=0).to(device)  # [B, L_tot_max, H_sent]
        attn_mask = mask1d(all_node_embeds, pad_id=0).to(device)       # [B, L_tot_max]
        # print(f"[process_batch_data] inputs_embeds.shape = {inputs_embeds.shape}")
        # print(f"[process_batch_data] attn_mask.shape = {attn_mask.shape}")


        max_len = max([b.shape[0] for b in all_node_bboxes])
        batch_size = len(all_node_bboxes)
        # print(f"[process_batch_data] max_len over batches = {max_len}")

        # global_box = torch.tensor([0, 0, 512, 512], dtype=torch.long, device=device)
        pad_box = torch.tensor([0, 0, 0, 0], dtype=torch.long, device=device)
        padded_bboxes = torch.zeros((batch_size, max_len, 4), dtype=torch.long, device=device)
        for i, b in enumerate(all_node_bboxes):
            L = b.shape[0]
            padded_bboxes[i, :L, :] = b
            if L < max_len:
                padded_bboxes[i, L:, :] = pad_box.unsqueeze(0).expand(max_len - L, 4)

        bboxes = padded_bboxes  # [B, L_tot_max, 4]
