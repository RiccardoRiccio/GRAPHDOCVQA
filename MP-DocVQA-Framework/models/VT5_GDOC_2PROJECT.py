...
# class VT5_GDOC_2PROJECT:
#     def __init__(self, config):
              

#         print("Using VT5_GDOC_2PROJECT")
...
#         print("Using 2 PROJECTOR LAYERS")
#         self.projection1 = nn.Linear(768, 1024).to(device)  # First layer: Original dimension -> 1024
#         self.projection2 = nn.Linear(1024, 768).to(device)              # Second layer: 1024 -> 768
...
#             # Concatenate GraphDoc embeddings
#             input_embeds = torch.cat([input_embeds, graphdoc_embeds], dim=1)        # [B, total_seq_len, d_model]
#             tensor_attention_mask = torch.cat([tensor_attention_mask, graphdoc_masks], dim=1)  # [B, total_seq_len]

#             print("BEFORE PROJECTION input_embeds and attention mask", input_embeds.shape, tensor_attention_mask.shape)

#             input_embeds = self.projection1(input_embeds)  # Transform to 1024 dimensions
#             input_embeds = self.activation(input_embeds)  # Apply activation
#             input_embeds = self.projection2(input_embeds)  # Keep it in 1024 dimensions
#             input_embeds = self.activation(input_embeds)  # Apply activation again (optional)

#             print("AFTER LAYERS input_embeds and attention mask", input_embeds.shape, tensor_attention_mask.shape)
...
class VT5_GDOC_2PROJECT(nn.Module):  # ✅ Now inherits from nn.Module
    def __init__(self, config):
        super(VT5_GDOC_2PROJECT, self).__init__()  # ✅ Call parent constructor
        
        print("Using VT5_GDOC_2PROJECT")
        self.batch_size = config['batch_size']
        
        # ✅ Register tokenizer as a buffer (not a parameter, but part of the module)
        self.tokenizer = T5Tokenizer.from_pretrained(config['model_weights'])
        
        # ✅ Register the main model as a submodule
        self.model = T5ForConditionalGeneration.from_pretrained(config['model_weights'])
        ...
        # ✅ Register embedding modules as submodules (remove manual .to(device))
        self.spatial_embedding = SpatialEmbeddings(t5_config)
        self.visual_embedding = VisualEmbeddings(t5_config)

        print("Using 2 PROJECTOR LAYERS")
        # ✅ Register projection layers as submodules (remove manual .to(device))
        self.projection1 = nn.Linear(768, 1024)
        self.projection2 = nn.Linear(1024, 768)
        self.activation = nn.ReLU()

        # ✅ Remove manual device management - PyTorch will handle this

    def parallelize(self):
        # ✅ Apply DataParallel to the entire module, not just self.model
    ...
    def prepare_inputs_for_vqa(self, question, words, boxes, images, answers=None, graphdoc_embeds=None, graphdoc_masks=None):
        ...
        # ✅ Use next(self.parameters()).device to get current device automatically
        ...
        if graphdoc_embeds is not None and graphdoc_masks is not None:
            ...
            # ✅ Apply projections (these are now proper nn.Module components)
            input_embeds = self.projection1(input_embeds)
            input_embeds = self.activation(input_embeds)
            input_embeds = self.projection2(input_embeds)
            input_embeds = self.activation(input_embeds)
    ...
    def forward(self, batch, return_pred_answer=False):
        ...
        if self.page_retrieval == 'logits':
            ...
            # Handle predictions if needed
            pred_answers, pred_answers_conf = self.get_answer_from_model_output(input_embeds, attention_mask) if return_pred_answer else (None, None)
