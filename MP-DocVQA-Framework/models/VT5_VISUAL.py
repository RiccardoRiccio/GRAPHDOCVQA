...
class VT5_SPATIALSCALED(nn.Module):
    def __init__(self, config):
        super(VT5_SPATIALSCALED, self).__init__()
        print("VT5 SPATIAL SCALED")
    ...
    def prepare_inputs_for_vqa(self, question, words, boxes, images, answers=None):
        ...
        for batch_idx in range(bs):
            ...
            for word, box in zip(words[batch_idx], boxes[batch_idx]):
                ...
                # Scale OCR bounding box values from [0, 1] to [0, 1000]
                scaled_box = [int(round(coord * 1000)) for coord in box]
                input_boxes.extend([scaled_box] * len(tokenized_word))  # Repeat the scaled box for each token corresponding to the word.
        ...
        # print("tensor boxes SHAPE", tensor_boxes.shape)
