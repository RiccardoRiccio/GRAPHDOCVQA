...
class VT5_LONGER(nn.Module):
    def __init__(self, config):
        super(VT5_LONGER, self).__init__()
        self.batch_size = config['batch_size']
        self.tokenizer = T5Tokenizer.from_pretrained(
            config['model_weights'],
            model_max_length=config.get('max_source_length', 1024)
        )

        self.model = T5ForConditionalGeneration.from_pretrained(config['model_weights'])

        self.max_source_length = config.get('max_source_length', 1024)
    ...
    def prepare_inputs_for_vqa(self, question, words, boxes, images, answers=None):
        ...
        # NEW: Define max_len using the tokenizer's model_max_length.
        ...
        for batch_idx in range(bs):
            # NEW: Pass max_length and truncation parameters.
            tokenized_prompt = self.tokenizer(
                prompt_text[batch_idx],
                max_length=max_len,
                truncation=False
            )
            ...
            for word, box in zip(words[batch_idx], boxes[batch_idx]):
                tokenized_word = self.tokenizer(
                    ...
                    max_length=max_len,
                    truncation=False
                ).input_ids[:-1]  # Tokenize the word and ignore eos_token
        ...
        # NEW: When tokenizing answers, pass max_length and truncation as well.
