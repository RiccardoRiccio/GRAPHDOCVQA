...
class VT5_GDOC_WEIGHTNOWORDS:
    def __init__(self, config):
        self.batch_size = config['batch_size']
         # Use the fine-tuned nowords checkpoint if provided.
        # The key 'pretrained_nowords' should point to the directory that contains the fine-tuned weights.
        # nowords_checkpoint = config.get("pretrained_nowords", config['model_weights'])
        nowords_checkpoint = config["model_weights"]


        print("checkpoints weights:", nowords_checkpoint)
        
        # Load the tokenizer and model from the provided checkpoint path.
        self.tokenizer = T5Tokenizer.from_pretrained(nowords_checkpoint)
        self.model = T5ForConditionalGeneration.from_pretrained(nowords_checkpoint)
        ...
        t5_config = CustomT5Config.from_pretrained(nowords_checkpoint)
