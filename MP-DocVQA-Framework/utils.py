#######################
# Updates:
# 1. Replaced the use of the '|' operator for dictionary merging (Python 3.9+ specific) 
#    with alternative approaches compatible with Python 3.8 and later.
#    - `config = config | {...}` changed to `config.update({...})` in `load_config()`.
#    - `config = load_config(...) | config` changed to `{**load_config(...), **config}` in `parse_config()`.
#    These changes ensure compatibility with a wider range of Python versions.
####################
...
def check_config(config):
    ...
    elif page_retrieval == 'none' and config['dataset_name'] not in ['SP-DocVQA', 'InfographicVQA_GRAPHDOC']:
