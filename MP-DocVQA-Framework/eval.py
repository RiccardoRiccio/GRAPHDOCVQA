...
# from datasets.SP_DocVQA_GDOC import singlepage_docvqa_collate_fn
...
def evaluate(data_loader, model, evaluator, **kwargs):
    ...
    for batch_idx, batch in enumerate(tqdm(data_loader)):
        ...
        # Debug prints for the first few samples in the current batch:
        for i in range(min(bs, 1)):
            print("=== Sample", i, "from batch", batch_idx, "===")
            print("Question:", batch['questions'][i])
            print("Ground Truth Answers:", batch['answers'][i])
            print("Predicted Answer:", pred_answers[i])
            print("Answer Confidence:", answer_conf[i] if answer_conf else "None")
            if 'answer_page_idx' in batch:
                print("Answer Page Index (GT):", batch.get('answer_page_idx')[i] if batch.get('answer_page_idx') is not None else "None")
            print("-------------------------")
