...
changed:
 train_dataset = build_dataset(config, 'train', max_samples=1000)
    val_dataset   = build_dataset(config, 'val', max_samples=500)
...
if __name__ == '__main__':
    ...
    # 2) Detect how many GPUs are _visible_ and wrap if >1
    ngpu = torch.cuda.device_count()
    if ngpu > 1:
        print(f"Running on {ngpu} GPUs via DataParallel")
        model = torch.nn.DataParallel(model)
    else:
        print(f"Running on {ngpu} GPU" + ("s" if ngpu != 1 else ""))
