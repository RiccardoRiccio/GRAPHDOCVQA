...
class InfographicsVQADataset(Dataset):
    def __init__(self, imdb_dir, images_dir, ocr_dir,  ocr_graphdoc_dir, split, dataset_kwargs, max_samples=None):
        ...
        # load the correct QA file based on the split
        if split == "train":
            qa_filename = "train_v1.0_withQT.json"
        elif split == "val":
            qa_filename = "val_v1.0_withQT.json"
        elif split == "test":
            qa_filename = "test_v1.0.json"
        else:
        ...
        for item in data['data']:
            ...
            # The JSON uses "image" (e.g. "documents/gxyd0217_17.png")
            # so extract just the filename:
            img_path_str = item['image']
            img_name     = Path(img_path_str).name           # → "gxyd0217_17.png"
            img_stem     = Path(img_path_str).stem           # → "gxyd0217_17"

            # And the OCR JSON lives in ocr_dir as "<stem>.json":
            ocr_fn       = f"{img_stem}.json"
            self.samples.append({
                'question_id': item['questionId'],
                'question': item['question'],
                'image_local_name': img_name,  # e.g. "20471.jpeg"
                'image_stem':  img_stem,  # e.g. "20471"
                'answers': item['answers'],
                'ocr_output_file': ocr_fn  # e.g. "20471.json"
            })
...
if __name__ == "__main__":
    ...
    config = {
        "imdb_dir": "/data2/users/rriccio/spdocvqa_qas",
        "images_dir": "/data2/users/rriccio/spdocvqa_images",
        "ocr_dir": "/data2/users/rriccio/spdocvqa_ocr",
        "ocr_graphdoc_dir": "/data2/users/rriccio/easyocr_spdocvqa"  # top-left, top-right, bottom right, bottom-left. entity-level OCR JSONs (with _easyocr.json naming)
    }
    split = "train"
    dataset_kwargs = {
