...
def visualize_results(image, results, level="paragraph"):
    ...
    destination_dir = "/home/rriccio/GraphDoc-VQA/easyocr_visualization"

    # Create the directory if it does not exist
    os.makedirs(destination_dir, exist_ok=True)

    # Generate output filename in the new folder
    base_name = os.path.basename(image)  # Extract filename from path
    file_name, _ = os.path.splitext(base_name)  # Remove extension
    output_path = os.path.join(destination_dir, f"{file_name}_ocr_visualization_{level}.png")
...
def main():
    try:
        ...
        image_path = "/data2/users/rriccio/infographic/infographicsvqa_images/10790.jpeg"
