...
class HybridVT5GraphDoc:
    ...
    def __init__(self, config):
        ...
        # 5) Re-enable gradients on the fresh decoder (to train it)
        # for param in self.vt5.model.decoder.parameters():
        #     param.requires_grad = True

        # print("\n--- Parameter Trainability Summary ---")

        # for name, param in self.vt5.model.named_parameters():
        #     print(f"{name}: {'Trainable' if param.requires_grad else 'Frozen'}")
        # print("--- End of Parameter Summary ---\n")

        # 5) Keep the same tokenizer from ProxyVT5
         # Load a fresh T5-base model
        print("[HybridVT5GraphDoc] Loading fresh T5-base model.")
        base_config = T5Config.from_pretrained("t5-base")
        base_full = T5ForConditionalGeneration.from_pretrained("t5-base", config=base_config)

        # Replace the encoder and decoder
        print("[HybridVT5GraphDoc] Replacing encoder and decoder with fresh T5-base components.")
        self.vt5.model.encoder = base_full.encoder
        self.vt5.model.decoder = base_full.decoder

        # Enable gradients for the new encoder and decoder
        for param in self.vt5.model.encoder.parameters():
            param.requires_grad = True
        for param in self.vt5.model.decoder.parameters():
            param.requires_grad = True

        # Keep the same tokenizer from ProxyVT5
    ...
    def forward(self, batch, return_pred_answer=False, return_confidence=False):
        ...
        # B) Pass input embeddings through the fresh encoder
        encoder_outputs = getattr(self.vt5.model, "module", self.vt5.model).encoder(
        ...
        outputs = self.vt5.model(
            inputs_embeds=question_enc,
            labels=labels,
            return_dict=True
        )

        # E) Generate predictions if required
        pred_answers, pred_conf = None, None
        if return_pred_answer:
            with torch.no_grad():
                base_model = getattr(self.vt5.model, "module", self.vt5.model)
                
                # Create `input_ids` for generation
                batch_size = question_enc.size(0)
                ...
                pred_answers = self.tokenizer.batch_decode(gen_out, skip_special_tokens=True)
...
class DocVQAQuestionDrivenDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Store the embedding file name
        embed_name = pt_path.name

        # Load OCR data
        words, normalized_boxes, ocr_file_name = self.load_ocr_data(image_stem)
        ...
        return {
            ...
            "image_name": [image_stem],             # List of one image name (stem)
            "embed_name": [embed_name],  # New key for embedding file name
            "ocr_file": [ocr_file_name]            # New key for the OCR file name
        }
    ...
    def load_ocr_data(self, stem):
        ...
        return all_words, all_boxes, ocr_path.name
...
def docvqa_collate_fn(batch):
    ...
    image_names = [b["image_name"][0] for b in batch]  # Extract image names
    embed_names = [b["embed_name"][0] for b in batch]  # Extract embedding file names
    ocr_files = [b["ocr_file"][0] for b in batch]  # Extract OCR file names
    ...
    return {
        ...
        "image_name": image_names,  # Include the list of image names
        "embed_name": embed_names,  # Include the embedding file names
        "ocr_file": ocr_files  # Add OCR file names to the final dictionary
    }
...
def main():
    ...
    config_file = "config/models/vt5_original.yml"
    ...
    #     print("  All Answers:", sample["all_answers"][0])
    #     print("  Target Answer:", sample["target_answers"][0])
    #     print("  Number of OCR words:", len(sample["words"][0]))
    #     print("  Number of OCR boxes:", len(sample["boxes"][0]))
    #     # Optionally, verify the precomputed document embeddings length:
    #     print("  Doc Embeds shape:", sample["doc_embeds"][0].shape)
    #     # If you wish, inspect the image source:
    #     print("  Image Name:", sample["image_name"][0])
    #     if sample["images"][0] is not None:
    #         print("  Image type:", type(sample["images"][0]))  # It will be a PIL image
    #     print("-----------------------------------")
    # # --- End Debug Samples ---
    ...
    val_loader = DataLoader(
        ...
        # --- Debug: Print a sample batch from the train_loader ---
    # for batch in train_loader:
    #     print("---- Sample Batch Debug ----")
    #     # For instance, print the first sample’s question and target answer.
    #     # Print overall batch shapes (assuming keys in the dictionary are lists or tensors)
    #     # for key, value in batch.items():
    #     #     # If the value is a list, print its length; if it is a tensor, print its shape.
    #     #     if isinstance(value, list):
    #     #         # Check if the first element is a tensor to then print its shape
    #     #         if len(value) > 0 and torch.is_tensor(value[0]):
    #     #             print(f"Batch key '{key}': list of {len(value)} elements, each with shape {value[0].shape}")
    #     #         else:
    #     #             print(f"Batch key '{key}': list of {len(value)} elements, values: {value}")
    #     #     elif torch.is_tensor(value):
    #     #         print(f"Batch key '{key}': tensor with shape {value.shape}")
    #     #     else:
    #     #         print(f"Batch key '{key}': {value}")
    #     print("Questions:", batch["questions"][0])
    #     print("Target Answer:", batch["target_answers"][0])
    #     print("Doc Embeds shape:", batch["doc_embeds"][0].shape)
    #     print("OCR words (first 5):", batch["words"][0][:5])
    #     print("OCR boxes (first 5):", batch["boxes"][0][:5])
    #     print("Image tensor shape:", batch["images"][0].shape)
    #     print("Image Name:", batch["image_name"][0])
    #     print("Embedding File Name:", batch["embed_name"][0])
    #     print("OCR File Name:", batch["ocr_file"][0])
        # break  # only print one batch for debugging
    # --- End Debug Batch ---
