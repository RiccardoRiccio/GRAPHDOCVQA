...
class SpatialEmbeddings(nn.Module):
    ...
    def __init__(self, config):
        ...
        # No longer hardcode self.device or move layers individually.
        ...
        # If you want to later experiment with h/w embeddings, you can add them here.
...
class VisualEmbeddings(nn.Module):
    ...
    def forward(self, images, page_idx_mask=None):
        ...
        # Move inputs to the same device as the image_model’s parameters.
        device = next(self.image_model.parameters()).device
        inputs = {k: v.to(device) for k, v in inputs.items()}
        vis_embeddings = self.image_model(**inputs).last_hidden_state  # [B, tokens, hidden]
        vis_embeddings = self.visual_emb_matcher(vis_embeddings)
        # Create attention mask on the same device as vis_embeddings.
        if page_idx_mask is not None:
            vis_attention_mask = torch.zeros(vis_embeddings.shape[:2], dtype=torch.long, device=vis_embeddings.device)
            vis_attention_mask[page_idx_mask] = 1
        else:
            vis_attention_mask = torch.ones(vis_embeddings.shape[:2], dtype=torch.long, device=vis_embeddings.device)
