...
class ProxyVT5(nn.Module):
    def __init__(self, config):
        super(ProxyVT5, self).__init__()
        # Do not force device inside the constructor.
        # Instead, just create the model and submodules.
        ...
        # Create submodules without forcing a device.
...
##############################################
        ...
        # Check if the model is wrapped in DataParallel and access the correct attribute
        if isinstance(self.model, torch.nn.DataParallel):
            semantic_embedding = self.model.module.shared(tensor_input_ids)
        else:
